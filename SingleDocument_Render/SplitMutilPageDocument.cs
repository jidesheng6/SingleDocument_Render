        ...
        public void SplitDocument()//分割Word文档
        {
            if(PublicShareClass.WindowStruct.TemplateFilePath!=string.Empty)
            {
                ...
                for (int i = 1; i < PublicShareClass.WindowStruct.TemplatePageCount + 1; i++)
                {
                    ...
                    if (i < 10)
                    {
                        SplitSaveName = $"00{i}测评人_{Path.GetFileName(PublicShareClass.WindowStruct.TemplateFilePath).Replace(".docx", "")}.docx";
                    }
                    else if (i >= 10 & i < 100)
                    {
                        SplitSaveName = $"0{i}测评人_{Path.GetFileName(PublicShareClass.WindowStruct.TemplateFilePath).Replace(".docx", "")}.docx";
                    }
                    else
                    {
                        SplitSaveName = $"{i}测评人_{Path.GetFileName(PublicShareClass.WindowStruct.TemplateFilePath).Replace(".docx", "")}.docx";
                    }
